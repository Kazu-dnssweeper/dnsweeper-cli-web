...
export class PrettyFormatter implements LogFormatter {
  ...
  format(entry: LogEntry): string {
    ...
    const level = this.colorize(entry.levelName, entry.levelName.toUpperCase().padEnd(7));
    const message = entry.message;
    const context = entry.context ? this.colorize('blue', `[${entry.context}]`) : '';
    const correlationId = entry.correlationId ? this.colorize('gray', `{${entry.correlationId}}`) : '';
    const duration = entry.duration ? this.colorize('gray', `(${entry.duration}ms)`) : '';
...
export class CustomFormatter implements LogFormatter {
  ...
  format(entry: LogEntry): string {
    ...
    formatted = formatted.replace('{{correlationId}}', entry.correlationId || '');
    formatted = formatted.replace('{{duration}}', entry.duration ? `${entry.duration}ms` : '');
    ...
    if (entry.meta) {
      ...
      Object.keys(entry.meta).forEach(key => {
        formatted = formatted.replace(`{{meta.${key}}}`, String(entry.meta![key]));
      });
    }
 ...
 * フォーマッターファクトリー
 */
export function createFormatter(type: 'json' | 'text' | 'pretty' | string): LogFormatter {
  switch (type) {
    case 'json':
