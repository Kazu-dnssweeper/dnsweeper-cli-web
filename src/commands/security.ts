...
import { DNSSecurityAnalyzer, SecurityThreat } from '../lib/dns-security-analyzer.js';
    ...
    .action(async (domain, options) => {
      ...
      try {
        ...
        const analyzer = new DNSSecurityAnalyzer(logger, {
          threatDetection: {
            enabledAnalyzers: ['malware', 'phishing', 'typosquatting', 'dga', 'fastflux', 'dns_hijacking', 'cache_poisoning', 'subdomain_takeover'],
        ...
        // リアルタイム監視モード
        if (options.realTime) {
          await startRealTimeMonitoring(analyzer, domains, records, options, logger);
        }
        ...
        logger.error('❌ DNS セキュリティ脅威検出でエラーが発生しました:', error);
        process.exit(1);
      }
    });
 ...
 * 分析対象ドメインの準備
 */
async function prepareDomains(domain: string | undefined, options: any, logger: Logger): Promise<string[]> {
  ...
  if (options.file) {
    if (!existsSync(options.file)) {
      throw new Error(`ドメインリストファイルが見つかりません: ${options.file}`);
    }
    ...
    logger.info(`📄 ${fileDomains.length} 個のドメインをファイルから読み込みました`);
  }
  
  if (domains.length === 0) {
    throw new Error('分析するドメインが指定されていません。ドメインを直接指定するか、--file オプションを使用してください。');
  }
...
async function getDNSRecords(domains: string[], options: any, logger: Logger) {
  if (options.records) {
    // CSV ファイルからレコードを読み込み
    logger.info('📄 CSVファイルからDNSレコードを読み込んでいます...', { file: options.records });
    ...
    // ドメインからDNSレコードを解決
    logger.info('🔍 DNSレコードを解決しています...', { domainCount: domains.length });
 ...
 * 脅威イベントリスナーの設定
 */
function setupThreatListeners(analyzer: DNSSecurityAnalyzer, logger: Logger, options: any) {
  // 脅威検出イベント
  ...
  analyzer.on('high-priority-threat', (threat: SecurityThreat) => {
    ...
    console.log(severityColor(`🚨 高優先度脅威: ${threat.domain} (${threat.type})`));
 ...
 * 脅威のフィルタリング
 */
function filterThreats(threats: SecurityThreat[], options: any): SecurityThreat[] {
 ...
 * 結果の出力
 */
async function outputResults(threats: SecurityThreat[], options: any, logger: Logger) {
...
function displayTableResults(threats: SecurityThreat[], logger: Logger) {
  ...
  threats.forEach((threat, index) => {
    ...
    console.log(chalk.gray(`   重要度: ${severityColor(threat.severity.toUpperCase())}`));
    console.log(chalk.gray(`   信頼度: ${threat.confidence}%`));
    console.log(chalk.gray(`   説明: ${threat.description}`));
    console.log(chalk.gray(`   検出時刻: ${new Date(threat.timestamp).toLocaleString()}`));
    console.log('');
  });
}
...
function displayDetailedResults(threats: SecurityThreat[], logger: Logger) {
  ...
  threats.forEach((threat, index) => {
    ...
    console.log(chalk.gray(`  関連DNSレコード: ${threat.evidence.dnsRecords.length}件`));
    console.log(chalk.gray(`  IP レピュテーション: ${threat.evidence.networkAnalysis.ipReputationScore}/100`));
    console.log(chalk.gray(`  ドメイン年数: ${threat.evidence.networkAnalysis.domainAge}日`));
    console.log(chalk.gray(`  証明書状態: ${threat.evidence.networkAnalysis.certificateStatus}`));
...
function displaySummaryResults(threats: SecurityThreat[], logger: Logger) {
  ...
  recentThreats.forEach((threat, index) => {
    const severityColor = getSeverityColor(threat.severity);
    const typeIcon = getThreatTypeIcon(threat.type);
    console.log(chalk.gray(`  ${index + 1}. ${typeIcon} ${threat.domain} (${severityColor(threat.severity)})`));
  });
}
 ...
 * 統計情報の表示
 */
function displayStatistics(analyzer: DNSSecurityAnalyzer, threats: SecurityThreat[], logger: Logger) {
...
async function startRealTimeMonitoring(
  ...
  const monitoringInterval = setInterval(async () => {
    try {
      const newThreats = await analyzer.analyzeSecurityThreats(domains, records);
 ...
 * 脅威データのエクスポート
 */
async function exportThreatData(analyzer: DNSSecurityAnalyzer, options: any, logger: Logger) {
...
function getThreatTypeIcon(type: string) {
  switch (type) {
    ...
    case 'typosquatting': return '🔤';
    case 'dga': return '🤖';
    case 'fastflux': return '⚡';
    case 'dns_hijacking': return '🔓';
    case 'cache_poisoning': return '☠️';
    case 'subdomain_takeover': return '🔗';
    default: return '⚠️';
  }
}