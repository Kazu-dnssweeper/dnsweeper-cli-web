...
import { AIDNSOptimizer, OptimizationContext, BusinessContext } from '../lib/ai-dns-optimizer.js';
...
export function createOptimizeCommand(): Command {
  ...
  command
    ...
    .option('-b, --business-context <file>', 'ビジネスコンテキストファイル（JSON）')
    ...
    .option('--priorities <priorities>', 'ビジネス優先度（カンマ区切り）', 'performance,security,reliability,cost')
...
async function getDNSRecords(domain: string, options: any, logger: Logger) {
  if (options.file) {
    // CSVファイルからレコードを読み込み
    logger.info('📄 CSVファイルからDNSレコードを読み込んでいます...', { file: options.file });
    ...
    const measure = globalPerformanceMonitor.startMeasurement('dns', 'bulk_resolve');
    ...
    try {
      ...
      measure(false, { domain, error: error instanceof Error ? error.message : String(error) });
      throw error;
    }
  }
}
...
function displayTableResults(suggestions: any[], logger: Logger) {
  ...
  suggestions.forEach((suggestion, index) => {
    ...
    console.log(chalk.gray(`   優先度: ${priorityColor(suggestion.priority.toUpperCase())}`));
    console.log(chalk.gray(`   分類: ${suggestion.type}`));
    console.log(chalk.gray(`   影響: パフォーマンス+${suggestion.impact.performance}, セキュリティ+${suggestion.impact.security}`));
    console.log(chalk.gray(`   難易度: ${suggestion.implementation.difficulty} (${suggestion.implementation.estimatedTime})`));
    console.log(chalk.gray(`   説明: ${suggestion.description}`));
    console.log(chalk.gray(`   対象: ${suggestion.affectedRecords.slice(0, 3).join(', ')}${suggestion.affectedRecords.length > 3 ? '...' : ''}`));
    console.log('');
  });
}
...
function displayDetailedResults(suggestions: any[], logger: Logger) {
  ...
  suggestions.forEach((suggestion, index) => {
    ...
    console.log(chalk.white.bold(`${index + 1}. ${typeIcon} ${suggestion.title}`));
    ...
    console.log(chalk.gray(`  パフォーマンス: ${suggestion.impact.performance}/10`));
    ...
    console.log(chalk.gray(`  難易度: ${suggestion.implementation.difficulty}`));
    console.log(chalk.gray(`  予想時間: ${suggestion.implementation.estimatedTime}`));
    ...
    if (suggestion.evidence.metrics.length > 0) {
      console.log(chalk.gray(`  メトリクス: ${suggestion.evidence.metrics.length}件`));
    }
    if (suggestion.evidence.riskFactors.length > 0) {
      console.log(chalk.gray(`  リスク要因: ${suggestion.evidence.riskFactors.join(', ')}`));
    }
...
function displaySummary(suggestions: any[], logger: Logger) {
  ...
  // 優先度別集計
  const priorityCount = suggestions.reduce((acc, s) => {
  ...
  // 総合影響度
  const totalImpact = suggestions.reduce((acc, s) => ({
  ...
  console.log(chalk.gray(`  コスト効果: ${totalImpact.cost >= 0 ? '+' : ''}${totalImpact.cost}`));
}
