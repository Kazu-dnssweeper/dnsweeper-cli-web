...
import { loadConfig, saveConfig, validateConfig, DnsSweeperConfig } from '../../src/lib/config.js';
import fs from 'fs';
...
describe('設定ファイル読み込み統合テスト', () => {
  ...
  describe('loadConfig', () => {
    ...
    it('設定ファイルから読み込む', async () => {
      const customConfig: DnsSweeperConfig = {
        dns: {
          timeout: 10000,
    ...
    it('親ディレクトリから設定ファイルを検索', async () => {
      const parentConfig: DnsSweeperConfig = {
        csv: {
  ...
  describe('深いマージ', () => {
    it('ネストされたオブジェクトを正しくマージ', async () => {
      const fileConfig: DnsSweeperConfig = {
        dns: {
          timeout: 6000,
          servers: ['1.1.1.1']
        },
