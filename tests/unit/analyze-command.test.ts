...
old-service.example.com,A,192.0.2.3,86400,,,`;
      ...
      expect(mockConsoleLog).toHaveBeenCalledWith(
        expect.stringContaining('Risk Analysis')
      );
...
medium-risk-dev.example.com,A,192.0.2.3,300,,,`;
      ...
      expect(mockConsoleLog).toHaveBeenCalledWith(
        expect.stringContaining('high-risk-test')
      );
    });
...
ns.example.com,NS,ns1.example.com,3600,,,`;
      ...
      expect(mockConsoleLog).toHaveBeenCalledWith(
        expect.stringContaining('A records')
      );
      expect(mockConsoleLog).toHaveBeenCalledWith(
        expect.stringContaining('MX records')
      );
    });
...
long.example.com,A,192.0.2.3,3600,,,`;
      ...
      // TTL分布統計が表示されることを確認
      expect(mockConsoleLog).toHaveBeenCalledWith(
        expect.stringContaining('TTL distribution')
      );
    });
  });
  ...
  describe('パフォーマンス', () => {
    it('大容量CSVファイルの分析', async () => {
      ...
      const rows = Array.from({ length: 1000 }, (_, i) => 
        `test${i}.example.com,A,192.0.2.${Math.floor(i / 254) + 1},${300 + (i % 100)},,,`
      );
...
low.example.com,A,192.0.2.4,3600,,,`;
      ...
      // 詳細レポートが生成されることを確認
      expect(mockConsoleLog).toHaveBeenCalledWith(
        expect.stringContaining('Analysis Report')
      );
      expect(mockConsoleLog).toHaveBeenCalledWith(
