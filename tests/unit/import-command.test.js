...
verbose.example.com,A,192.0.2.1,300,`;
            ...
            expect(mockConsoleLog).toHaveBeenCalledWith(expect.stringContaining('Processing'));
        });
        it('フォーマット自動検出', async () => {
            const csvContent = `Name,Type,Content,TTL,Priority
auto.example.com,A,192.0.2.1,300,`;
    ...
    describe('大容量ファイル処理', () => {
        it('大容量CSVファイルをストリーミング処理', async () => {
            ...
            const rows = Array.from({ length: 1000 }, (_, i) => `test${i}.example.com,A,192.0.2.${Math.floor(i / 254) + 1},300,`);
...
another.example.com,A,192.0.2.2,300,`;
    ...
    describe('進捗表示', () => {
        it('進捗情報を適切に表示', async () => {
            const headers = 'Name,Type,Content,TTL,Priority';
            const rows = Array.from({ length: 100 }, (_, i) => `progress${i}.example.com,A,192.0.2.1,300,`);
            const csvContent = [headers, ...rows].join('\n');
            const filePath = await createTempCsvFile(csvContent);
            const { importCommand } = await import('../../src/commands/import.js');
            await importCommand.action(filePath, {
            ...
            // 進捗情報が出力されることを確認
            expect(mockConsoleLog).toHaveBeenCalledWith(expect.stringContaining('records processed'));
        });
    });
    describe('エンコーディング対応', () => {
...
//# sourceMappingURL=import-command.test.js.map