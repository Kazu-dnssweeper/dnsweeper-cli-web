...
import { StructuredLogger, LogLevel, LOG_LEVEL_NAMES, ConsoleTransport, FileTransport, createLogger, getLogger, setGlobalLogger, logMethod } from '../../src/lib/structured-logger.js';
describe('StructuredLogger', () => {
    ...
    describe('ConsoleTransport', () => {
        ...
        it('メタデータを含む出力', () => {
            ...
            transport.write(entry);
            expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('{"userId":123,"action":"login"}'));
        });
    });
    ...
    describe('StructuredLogger', () => {
        ...
        it('デフォルトメタデータを設定', () => {
            ...
            expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('"service":"test"'));
            consoleLogSpy.mockRestore();
        });
        it('コンテキストスタック管理', () => {
        ...
        it('プロファイル機能（成功）', async () => {
            ...
            expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('Starting: test-function'));
            expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('Completed: test-function'));
            consoleLogSpy.mockRestore();
        });
        it('プロファイル機能（エラー）', async () => {
            ...
            expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('Starting: test-function'));
            expect(consoleErrorSpy).toHaveBeenCalledWith(expect.stringContaining('Failed: test-function'));
            consoleLogSpy.mockRestore();
            consoleErrorSpy.mockRestore();
        });
        it('子ロガーの作成', () => {
            ...
            expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('"service":"parent"'));
            expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('"module":"child"'));
            expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('child-context'));
            consoleLogSpy.mockRestore();
        });
    });
    describe('ログレベル解析', () => {
    ...
    describe('logMethodデコレーター', () => {
        let TestClass = (() => {
            ...
            let _errorMethod_decorators;
            return class TestClass {
                static {
                    const _metadata = typeof Symbol === "function" && Symbol.metadata ? Object.create(null) : void 0;
                    _testMethod_decorators = [logMethod(LogLevel.DEBUG, true, true)];
                    _errorMethod_decorators = [logMethod(LogLevel.DEBUG)];
                    __esDecorate(this, null, _testMethod_decorators, { kind: "method", name: "testMethod", static: false, private: false, access: { has: obj => "testMethod" in obj, get: obj => obj.testMethod }, metadata: _metadata }, null, _instanceExtraInitializers);
                    __esDecorate(this, null, _errorMethod_decorators, { kind: "method", name: "errorMethod", static: false, private: false, access: { has: obj => "errorMethod" in obj, get: obj => obj.errorMethod }, metadata: _metadata }, null, _instanceExtraInitializers);
        ...
        it('メソッド呼び出しをログに記録', async () => {
            ...
            expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('Method called: TestClass.testMethod'));
            expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('Method completed: TestClass.testMethod'));
            consoleLogSpy.mockRestore();
        });
        it('メソッドエラーをログに記録', async () => {
            ...
            expect(consoleErrorSpy).toHaveBeenCalledWith(expect.stringContaining('Method failed: TestClass.errorMethod'));
            consoleLogSpy.mockRestore();
            consoleErrorSpy.mockRestore();
        });
    });
