...
describe('Route53Client', () => {
  ...
  describe('getHostedZone', () => {
    ...
    it('ゾーンIDのプレフィックスを適切に処理', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith(
        expect.stringContaining('/hostedzone/Z123456789'),
        expect.any(Object)
      );
    });
  });
  ...
  describe('listResourceRecordSets', () => {
    ...
    it('フィルターオプションを適切に適用', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith(
        expect.stringContaining('maxitems=100'),
        expect.any(Object)
      );
    });
  });
  ...
  describe('convertRoute53ToCSVRecords', () => {
    it('Route53レコードを正しくCSVレコードに変換', () => {
      const route53Records: Route53Record[] = [
        {
          Name: 'example.com.',
          Type: 'A',
          TTL: 300,
  ...
  describe('importRecords', () => {
    ...
    it('大量レコードを適切にバッチ分割', async () => {
      ...
      const csvRecords: ICSVRecord[] = Array.from({ length: 150 }, (_, i) => ({
